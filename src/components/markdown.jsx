...
class Emitter {
  ...
  startScope(name) {
    const node = {
      type: "span",
      props: {
        class: name.split(".").map((c, i) => `hljs-${c}${"_".repeat(i)}`).join(" "),
        children: [],
      },
    };
    this.top.push(node);
    this.stack.push(node.props.children);
  }
...
Emitter.prototype.openNode = Emitter.prototype.startScope;
Emitter.prototype.closeNode = Emitter.prototype.endScope;
...
function Blockquote({ children }) {
  ...
  for (const alert in alerts) {
    const str = text.substring(2, 2 + alert.length).toUpperCase();
    if (str !== alert || alerts[str] === undefined) {
      continue;
    }
    children[0].props.children = text.substring(3 + alert.length);
    return (
      <blockquote class={`alert alert-${alert.toLowerCase()}`}>
        <header>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" height="16" width="16">
            <path d={alerts[str]} />
          </svg>
          {alert[0] + alert.slice(1).toLowerCase()}
        </header>
        {children}
      </blockquote>
    );
